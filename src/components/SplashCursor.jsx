...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    if (!gl || !ext || !ext.halfFloatTexType || !ext.formatRGBA || !ext.formatRG || !ext.formatR) {
      return;
    }
    ...
    function isPointerInsideProfileCard(clientX, clientY) {
      const hit = document.elementFromPoint(clientX, clientY);
      return !!hit?.closest('.about-profile-card');
    }

    function syncPointerWithoutSplat(pointer, posX, posY) {
      const tx = posX / canvas.width;
      const ty = 1.0 - posY / canvas.height;
    ...
    function handleMouseDown(e) {
      if (isPointerInsideProfileCard(e.clientX, e.clientY)) {
        return;
      }
    ...
    function handleMouseMove(e) {
      ...
      if (isPointerInsideProfileCard(e.clientX, e.clientY)) {
        syncPointerWithoutSplat(pointer, posX, posY);
        return;
      }
    ...
    function handleTouchStart(e) {
      ...
      for (let i = 0; i < touches.length; i += 1) {
        if (isPointerInsideProfileCard(touches[i].clientX, touches[i].clientY)) {
          continue;
        }
